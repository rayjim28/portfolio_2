...
const Projects = () => {
  ...
  const projectsData: ProjectType[] = [
    // First project details
    {
      ...
      "link": "https://portfolio-2-khaki.vercel.app/",
      "theme": "dark"
    },
    ...
    <section ref={projectsRef} id="projects" className="relative py-6 sm:py-12 px-4 text-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
        {projectsData.map((project) => (
          <motion.div
            key={project.title}
            className="rounded-lg shadow-md border-2 bg-[color] p-4 transition-all cursor-pointer" // Replace [color] with my chosen color
      ...
      {showModal && currentProject && (
        ...
        >
          <Modal onClose={() => setShowModal(false)}>
            ...
            <Image src={currentProject.image} width={300} height={200} alt={currentProject.title} />
            <p className="mt-2 mb-4">{currentProject.description}</p>
            <a href={currentProject.link} target="_blank" rel="noopener noreferrer" className="inline-block bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition duration-300">
              Visit Site
            </a>
          </Modal>
        </motion.div>
